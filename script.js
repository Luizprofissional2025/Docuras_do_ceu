...
(function () {
  ...
  /* ---------------------------------------------------------
     LOGIN OBRIGATÓRIO (Google) + CADASTRO COMPLEMENTAR
     Fluxo: 1) usuário entra com Google -> POST /api/auth-google
  ...
  function updateUserChip(user) {
    ...
    document.getElementById("userChipAvatar").src = user.picture || "images/logo.png";
  ...
  async function handleGoogleCredential(response) {
    ...
    try {
      ...
      if (!res.ok) {
        authLoginError.textContent = data.error || "Não foi possível entrar com o Google. Tente de novo.";
        return;
      }
